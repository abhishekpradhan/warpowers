...
- **W3D models**: `tools/genw3d.py` generates engine-valid meshes (byte spec from the GPL reader; see the tool). Verified loading through: prototype registration → scene add → frustum → DX8 batch registration → draw-task submission. **Open issue:** submitted draws produce no pixels on the macOS port (both untextured and textured/UV variants) — isolated to the D3D8→DXVK layer; upstream GeneralsX fights adjacent bugs there (#166 black skinned meshes fixed via mipmap downsampling in d3dx8_compat.cpp, #240 regression open). Next levers: diff our draw state vs a retail mesh's, or upstream-issue the minimal repro.
