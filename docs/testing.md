...
| `python3 -m unittest discover -s tests -p 'test_*.py'` | Packaging and tool tests (46): `test_staging.py` covers repeatable hashes, static files and link-preview URLs, content references and notices, UI/save compatibility identity, engine-glue release identity, protected output paths, oversized-candidate rejection and release source-URL rules; `test_tools.py` covers the TGA, W3D and map readers, the generators failing closed on a name collision, and CLI conflicts | The engine build itself |
