...
| `blender/build_polish.py` | `polish_assets.json`, `support_kit.py` | 47 catalog models: `data/Art/W3D/*.w3d`, raw bakes in `data/Art/Textures/wp_*.tga`, portraits + manifest in `--review`; refreshes `polish_assets.json` only when `--data` is the repository `data/` | Yes (`--data DIR`; `--only a,b`; `--check` writes nothing) |
| `blender/build_mercc.py`, `build_jakcp.py`, `build_meridian_base.py`, `build_jackal_base.py` | `_bootstrap.py` palette, `wp_pipeline.py` | The retained HQ/base/builder models and their raw bakes in `data/Art/W3D` and `data/Art/Textures` | Yes (`--data DIR` or `--scratch DIR`) |
...
| `blender/_bootstrap.py`, `wp_pipeline.py`, `support_kit.py`, `legacy_contracts.py` | libraries: plugin registration and `--` args, bake/composite/export, support-kit geometry, ownership panels | — | — |
...
# All 47 catalog models into data/ (raw bakes; then optimize_art.py):
...
# Retained base kits into a flat review directory (or --data data):
$BLENDER --background --factory-startup --python tools/blender/build_mercc.py -- --scratch /tmp/wp-kit
$BLENDER --background --factory-startup --python tools/blender/build_jakcp.py -- --scratch /tmp/wp-kit
$BLENDER --background --factory-startup --python tools/blender/build_meridian_base.py -- --scratch /tmp/wp-kit
$BLENDER --background --factory-startup --python tools/blender/build_jackal_base.py -- --scratch /tmp/wp-kit
...
| `build_polish.py --check` | 47/47 models: W3D and optimized TGA identical |
...
| `build_mercc.py`, `build_jakcp.py`, `build_meridian_base.py`, `build_jackal_base.py` | **not** reproducible: the committed files were exported by earlier revisions of these scripts and of Blender; a rerun differs by float rounding in the W3D vertices (and `merpp01` by its vertex split count) and by pixels in the bakes. The scripts run headless and produce the same bytes at every revision since, so the drift is historical, not a regression. |
