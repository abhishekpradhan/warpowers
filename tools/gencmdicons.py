...
Written in the committed storage form (RLE, top-left), so a rerun is
byte-identical.

python3 tools/gencmdicons.py [--data DIR | --out FILE]
"""
import argparse
...
def main(argv=None):
    ...
    parser.add_argument('--data', type=Path, default=Path(__file__).resolve().parents[1] / 'data',
                        help='dataset root; writes Art/Textures/wp_cmdglyphs.tga (default: the repository data/)')
    parser.add_argument('--out', type=Path, help='explicit output file instead of the dataset location')
    args = parser.parse_args(argv)
    target = args.out or args.data / 'Art/Textures/wp_cmdglyphs.tga'
    target.parent.mkdir(parents=True, exist_ok=True)
    size = wp_tga.write(target, glyph_sheet())
